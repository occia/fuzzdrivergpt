...
// the following function fuzzes parse_refer_to_header based on the above API usages
extern int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
// First, we initialize some variables to be used in the fuzz driver.
	char tbuf[4 * BUF_SIZE];
	sip_msg_t tmsg = {};
	tmsg.buf = tbuf;

// Next, we check if the size of the input data is too large.
	if (Size >= 4 * BUF_SIZE - 2) {
		return 0;
	}

// Next, we copy the input data into the tbuf variable and set the length of the sip message buffer.
	memcpy(tbuf, Data, Size);
	tbuf[Size] = '\0';
	tmsg.len = Size;

// We print out some debugging information about the sip message buffer.
	LM_INFO("data - start: %p - end: %p - len: %d\n", tmsg.buf,
			tmsg.buf + tmsg.len, tmsg.len);
	LM_INFO("data - content: [[%.*s]]\n", tmsg.len, tmsg.buf);

// We call parse_msg to parse the sip message buffer and fill out the sip_msg structure with the parsed information.
	if (parse_msg(tbuf, tmsg.len, &tmsg) < 0) {
		goto cleanup;
	}

// We parse the various headers of the sip message using the relevant parse_* functions.
	parse_headers(&tmsg, HDR_EOH_F, 0);
	parse_sdp(&tmsg);
	parse_from_header(&tmsg);
	parse_from_uri(&tmsg);
...
// We retrieve the source URI and socket information from the sip message.
	str uri;
	get_src_uri(&tmsg, 0, &uri);
...
// Finally, we free the sip message buffer and return.
cleanup:
	free_sip_msg(&tmsg);
