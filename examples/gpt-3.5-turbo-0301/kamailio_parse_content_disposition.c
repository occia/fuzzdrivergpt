...
// the following function fuzzes parse_content_disposition based on the above API usages
extern int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
// First, we need to initialize some variables to hold the SIP message data.
char tbuf[4*BUF_SIZE];
sip_msg_t tmsg = { };
tmsg.buf = tbuf;

// Copy the input data into the tbuf variable.
...
// Use the parse_content_disposition function to parse the message and extract content disposition information.
if (parse_msg(tbuf, tmsg.len, &tmsg) >= 0) {
    parse_headers(&tmsg, HDR_EOH_F, 0);
    parse_content_disposition(&tmsg);
}

// Clean up after processing the message.
free_sip_msg(&tmsg);
