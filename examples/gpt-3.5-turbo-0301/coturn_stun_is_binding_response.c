...
// First we need to allocate a stun_buffer and copy the input data to its buffer
  stun_buffer buf;
  memcpy(buf.buf, Data, Size);
  buf.len = Size;
  
  // Next, we need to initialize some variables that we will use later
  int sockfd = 0;
  ...
  // Then we need to call the stun_is_binding_response function with the buf to be fuzzed
