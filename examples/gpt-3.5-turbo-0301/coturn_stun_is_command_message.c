...
// First, we need to allocate memory for the stun_buffer struct and copy the input data into it
  stun_buffer *buf = (stun_buffer *)malloc(sizeof(stun_buffer));
  memcpy(buf->buf, Data, Size);
  buf->len = Size;

  // Call stun_is_command_message and print the result
  int is_command_message = stun_is_command_message(buf);
  printf("is_command_message: %d\n", is_command_message);
