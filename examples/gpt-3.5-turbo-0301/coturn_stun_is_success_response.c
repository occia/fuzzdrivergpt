...
// First, we create a stun_buffer object and copy the input data into its buffer.
  stun_buffer buf;
  memcpy(buf.buf, Data, Size);
  buf.len = Size;

  // Then we call stun_is_success_response and check its return value.
  int success = stun_is_success_response(&buf);

  // Finally, we print the result of the function as a debug message.
  printf("stun_is_success_response returned %d\n", success);
