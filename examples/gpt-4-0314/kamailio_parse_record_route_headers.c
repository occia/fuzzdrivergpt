...
// the following function fuzzes parse_record_route_headers based on the above API usages
extern int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
// Create a buffer to store the data
    char tbuf[4*BUF_SIZE];

    // Create a sip_msg_t structure
    sip_msg_t tmsg = { };
    ...
    // Set the buffer of the message to the buffer
    tmsg.buf = tbuf;
    ...
    // Fuzz the parse_record_route_headers function
    parse_record_route_headers(&tmsg);
