...
sip_msg_t tmsg = { 0 };
  char tbuf[4*1024]; // 4KB buffer for holding the input data

  if (Size >= sizeof(tbuf) - 1) {
    return 0; // input is too big for our buffer, so we ignore this testcase
  }

  // copy input data to our buffer and null-terminate it
  memcpy(tbuf, Data, Size);
  tbuf[Size] = 0;

  // set the buffer and length in the SIP message
  tmsg.buf = tbuf;
  tmsg.len = (int)Size;

  // parse various headers and fields from the SIP message
  parse_msg(tmsg.buf, tmsg.len, &tmsg);
  parse_headers(&tmsg, HDR_EOH_F, 0);
  parse_sdp(&tmsg);
  parse_from_header(&tmsg);
  parse_from_uri(&tmsg);
  ...
  // fuzz the parse_refer_to_header function with the SIP message
  parse_refer_to_header(&tmsg);
