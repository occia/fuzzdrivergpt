...
// check if there is enough data to work with
  if (Size < 2) {
    return 0;
  }

  // allocate memory for input data and copy the content
  ...
  parse_headers(&fmsg, HDR_EOH_F, 0);
  parse_privacy(&fmsg);
  parse_from_header(&fmsg);
  parse_from_uri(&fmsg);
  parse_to_header(&fmsg);
  parse_to_uri(&fmsg);
  parse_contact_headers(&fmsg);
  parse_record_route_headers(&fmsg);
  parse_route_headers(&fmsg);
  parse_refer_to_header(&fmsg);
  parse_pai_header(&fmsg);
  parse_diversion_header(&fmsg);
  parse_content_disposition(&fmsg);
  parse_identityinfo_header(&fmsg);

  str uri;
  get_src_uri(&fmsg, 0, &uri);

  str ssock;
  get_src_address_socket(&fmsg, &ssock);
