...
// @ examples of API usage from fuzz_die_cu_offset.c-LLVMFuzzerTestOneInput
...
//extern int dwarf_offdie_b(Dwarf_Debug dw_dbg,Dwarf_Off dw_offset,Dwarf_Bool dw_is_info,Dwarf_Die * dw_return_die,Dwarf_Error * dw_error);
//extern void dwarf_dealloc_die(Dwarf_Die dw_die);
...
//extern int dwarf_get_version_of_die(Dwarf_Die dw_die,Dwarf_Half * dw_version,Dwarf_Half * dw_offset_size);
...
//extern int dwarf_CU_dieoffset_given_die(Dwarf_Die dw_die,Dwarf_Off * dw_return_offset,Dwarf_Error * dw_error);
      ...
      if (res == DW_DLV_OK) {
        ...
        if (res == DW_DLV_OK) {
          ...
          if (res == DW_DLV_OK) {
            ...
            if (res == DW_DLV_OK) {
              Dwarf_Half offset_size = 0;
              res = dwarf_get_version_of_die(cudie, &version_stamp, &offset_size);
              if (res == DW_DLV_OK) {
                dwarf_dealloc_die(cudie);
              }
            }
