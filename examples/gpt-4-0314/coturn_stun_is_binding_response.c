...
// the following function fuzzes stun_is_binding_response based on the above API usages
extern int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
// Create a stun_buffer
    stun_buffer buf;

    // Ensure buffer isn't bigger than the maximum size
    if (Size > sizeof(buf.buf)) {
        return 0;
    }

    // Clear the stun_buffer
    memset(&buf, 0, sizeof(buf));

    // Copy the data into the stun_buffer
    ...
    // Test the fuzzed data
    stun_is_command_message(&buf);
    stun_is_success_response(&buf);
    stun_is_binding_response(&buf);
    stun_is_error_response(&buf, &err_code, err_msg, err_msg_size);
    stun_is_request(&buf);

    ioa_addr addr;
    addr_set_any(&addr);
    stun_attr_get_first_addr(&buf, STUN_ATTRIBUTE_XOR_MAPPED_ADDRESS, &addr, NULL);
    stun_attr_get_first_by_type_str(buf.buf, buf.len, STUN_ATTRIBUTE_OTHER_ADDRESS);
