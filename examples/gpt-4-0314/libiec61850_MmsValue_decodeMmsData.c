...
MmsValue *decodedMmsValue;
  int endBufPos = 0;

  // Fuzz MmsValue_decodeMmsData with input data and size
  decodedMmsValue = MmsValue_decodeMmsData((uint8_t *)Data, 0, Size, &endBufPos);

  // If decoding is successful, delete the decoded value to free memory
  if (decodedMmsValue != NULL) {
    MmsValue_delete(decodedMmsValue);
  }
