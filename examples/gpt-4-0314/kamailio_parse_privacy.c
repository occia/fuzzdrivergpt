...
// the following function fuzzes parse_privacy based on the above API usages
...
if (Size == 0)
    ...
    sip_msg_t msg = { };
    char buf[4*BUF_SIZE];
    size_t buf_size = Size < 4*BUF_SIZE ? Size : 4*BUF_SIZE-1;
    ...
    sip_uri_t *from_uri = parse_from_uri(&msg);
    sip_uri_t *to_uri = parse_to_uri(&msg);
