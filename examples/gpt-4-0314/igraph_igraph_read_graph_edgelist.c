...
if (Size < 2) {
    ...
    // Create a temporary file with Data as its content
    file = tmpfile();
    if (file == NULL) {
        return 0;
    }
    ...
    // Test the various igraph file reading functions
    if (igraph_read_graph_edgelist(&graph, file, 0, IGRAPH_UNDIRECTED) == IGRAPH_SUCCESS) {
        igraph_destroy(&graph);
        rewind(file);
    }
    if (igraph_read_graph_ncol(&graph, file, NULL, 1, IGRAPH_ADD_WEIGHTS_IF_PRESENT, IGRAPH_UNDIRECTED) == IGRAPH_SUCCESS) {
        igraph_destroy(&graph);
        rewind(file);
    }
    if (igraph_read_graph_lgl(&graph, file, 1, IGRAPH_ADD_WEIGHTS_IF_PRESENT, IGRAPH_UNDIRECTED) == IGRAPH_SUCCESS) {
        igraph_destroy(&graph);
        rewind(file);
    }
    ...
    if (igraph_read_graph_pajek(&graph, file) == IGRAPH_SUCCESS) {
        igraph_destroy(&graph);
        rewind(file);
    }
    if (igraph_read_graph_dl(&graph, file, IGRAPH_UNDIRECTED) == IGRAPH_SUCCESS) {
        igraph_destroy(&graph);
        rewind(file);
    }
    ...
    if (igraph_read_graph_graphdb(&graph, file, IGRAPH_UNDIRECTED) == IGRAPH_SUCCESS) {
        igraph_destroy(&graph);
        rewind(file);
    }
    if (igraph_read_graph_dimacs_flow(&graph, file, NULL, NULL, NULL, NULL, NULL, IGRAPH_DIRECTED) == IGRAPH_SUCCESS) {
        igraph_destroy(&graph);
        rewind(file);
    }
