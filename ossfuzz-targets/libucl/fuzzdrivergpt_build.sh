...
cd /src/libucl
make clean
./configure --prefix=/src/libucl/fuzzdrivergpt-install
make install
EOF
set +eux
...
cp -r /src/libucl/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/libucl/fuzzdrivergpt-install/lib ${INSTALL}/lib