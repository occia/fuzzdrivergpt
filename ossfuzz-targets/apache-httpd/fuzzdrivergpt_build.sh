...
make clean
./configure --with-included-apr --enable-pool-debug --prefix=/src/httpd/fuzzdrivergpt-install
make install
EOF
set +eux
...
cp -r /src/httpd/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/httpd/fuzzdrivergpt-install/lib ${INSTALL}/lib
