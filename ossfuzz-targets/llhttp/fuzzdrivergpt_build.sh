...
PREFIX=/src/llhttp/fuzzdrivergpt-install make -j$(nproc) install
EOF
set +eux
...
cp -r /src/llhttp/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/llhttp/fuzzdrivergpt-install/lib ${INSTALL}/lib
