...
cd /src/wavpack
CFLAGS="$CFLAGS -fno-sanitize=signed-integer-overflow" ./configure --disable-apps --disable-shared --enable-static --prefix=/src/wavpack/fuzzdrivergpt-install
make install
...
cp -r /src/wavpack/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/wavpack/fuzzdrivergpt-install/lib ${INSTALL}/lib
