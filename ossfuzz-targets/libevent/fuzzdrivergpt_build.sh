...
cmake -DEVENT__DISABLE_MBEDTLS=ON \
      ...
      --install-prefix=/src/libevent/fuzzdrivergpt-install ../
make -j$(nproc)
make install
EOF
set +eux
...
cp -r /src/libevent/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/libevent/fuzzdrivergpt-install/lib ${INSTALL}/lib
