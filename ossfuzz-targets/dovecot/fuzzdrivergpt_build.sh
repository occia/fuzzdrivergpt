...
make clean
./configure PANDOC=false --with-fuzzer=clang --prefix=/src/dovecot/fuzzdrivergpt-install
make install
EOF
set +eux
...
cp -r /src/dovecot/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/dovecot/fuzzdrivergpt-install/lib ${INSTALL}/lib
