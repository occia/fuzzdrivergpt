...
set -eux
# FILL HERE
# /src/civetweb/fuzztest/build.sh
cat >> /src/build.sh << 'EOF'
make clean
make  install-headers install-lib install PREFIX=/src/civetweb/fuzzdrivergpt-install
EOF
set +eux
...
cp -r /src/civetweb/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/civetweb/fuzzdrivergpt-install/lib ${INSTALL}/lib
