...
mkdir build1
cd build1
cmake  -DBUILD_SHARED_LIBS=0 -DAVIF_CODEC_DAV1D=1 -DAVIF_LOCAL_DAV1D=1 --install-prefix=/src/libavif/fuzzdrivergpt-install  ..
...
cp -r /src/libavif/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/libavif/fuzzdrivergpt-install/lib ${INSTALL}/lib
