...
cd /src/libsodium
make clean
./configure --enable-static LDFLAGS="$CXXFLAGS" --prefix=/src/libsodium/fuzzdrivergpt-install
...
cp -r /src/libsodium/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/libsodium/fuzzdrivergpt-install/lib ${INSTALL}/lib
