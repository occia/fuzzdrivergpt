...
mkdir build1
cd build1
cmake  -DENABLE_TESTING=OFF --install-prefix=/src/mbedtls/fuzzdrivergpt-install ..
...
cp -r /src/mbedtls/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/mbedtls/fuzzdrivergpt-install/lib ${INSTALL}/lib
