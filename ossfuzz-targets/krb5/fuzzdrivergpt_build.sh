...
sed -i 's#make#make -j36#g' /src/build.sh
sed -i 's#configure#configure --prefix=/src/krb5/fuzzdrivergpt-install#g' /src/build.sh
set -eux
# FILL HERE
...
cp -r /src/krb5/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/krb5/fuzzdrivergpt-install/lib ${INSTALL}/lib
