...
cd /src/hoextdown
make install -j$(nproc) PREFIX=/src/hoextdown/fuzzdrivergpt-install
...
cp -r /src/hoextdown/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/hoextdown/fuzzdrivergpt-install/lib ${INSTALL}/lib
