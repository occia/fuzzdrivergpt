...
cd /src/lcms
./configure --enable-shared=no --enable-static --prefix=/src/lcms/fuzzdrivergpt-install
...
cp -r /src/lcms/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/lcms/fuzzdrivergpt-install/lib ${INSTALL}/lib
