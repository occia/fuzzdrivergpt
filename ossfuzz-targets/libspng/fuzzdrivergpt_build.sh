...
mkdir /src/libspng/cbuild
cd /src/libspng/cbuild
apt install -y libz-dev
cmake .. --install-prefix=/src/libspng/fuzzdrivergpt-install
make -j$(nproc)
make install
...
cp -r /src/libspng/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/libspng/fuzzdrivergpt-install/lib ${INSTALL}/lib
