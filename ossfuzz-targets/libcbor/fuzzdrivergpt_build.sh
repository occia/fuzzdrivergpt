...
mkdir nbuild
cd nbuild
cmake -D CMAKE_BUILD_TYPE=Debug -D CMAKE_INSTALL_PREFIX="/src/libcbor/fuzzdrivergpt-install" -D SANITIZE=OFF ..
make "-j$(nproc)"
make install
EOF
set +eux
...
cp -r /src/libcbor/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/libcbor/fuzzdrivergpt-install/lib ${INSTALL}/lib
