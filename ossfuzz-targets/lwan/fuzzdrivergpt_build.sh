...
mkdir build
cd build
cmake -GNinja --install-prefix=/src/lwan/fuzzdrivergpt-install  ..
ninja install
...
cp -r /src/lwan/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/lwan/fuzzdrivergpt-install/lib ${INSTALL}/lib
