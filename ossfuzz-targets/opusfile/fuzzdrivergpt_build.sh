...
cd /src/opusfile
make clean
./configure --enable-static --disable-shared --disable-doc --enable-assertions --prefix=/src/opusfile/fuzzdrivergpt-install
make install
...
cp -r /src/opusfile/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/opusfile/fuzzdrivergpt-install/lib ${INSTALL}/lib
