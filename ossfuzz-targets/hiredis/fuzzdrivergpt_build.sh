...
cd /src/hiredis
make clean
make -j36 PREFIX=/src/hiredis/fuzzdrivergpt-install  USE_SSL=0 install
set -eux
EOF
set +eux
...
cp -r /src/hiredis/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/hiredis/fuzzdrivergpt-install/lib ${INSTALL}/lib
