...
sed '/^make$/ s/#/make -j $(nproc)/' /src/build.sh
set -eux
# FILL HERE
cat >> /src/build.sh << 'EOF'

cd /src/lua
mkdir fuzzdrivergpt-install
cd fuzzdrivergpt-install
mkdir lib
mkdir include
cp /src/lua/liblua.a ./lib/liblua.a
cp ../*.h ./include
EOF
set +eux
...
cp -r /src/lua/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/lua/fuzzdrivergpt-install/lib ${INSTALL}/lib
