...
mkdir build1
cd build1
cmake -DBUILD_SHARED_LIBS=OFF -DENABLE_CJSON_TEST=OFF -DCMAKE_VERBOSE_MAKEFILE=ON --install-prefix=/src/cjson/fuzzdrivergpt-install   ..
...
cp -r /src/cjson/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/cjson/fuzzdrivergpt-install/lib ${INSTALL}/lib
