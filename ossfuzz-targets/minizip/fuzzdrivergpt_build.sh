...
cd /src/minizip-ng
cmake . -DCMAKE_C_FLAGS="$CFLAGS" -DCMAKE_CXX_FLAGS="$CXXFLAGS" -DMZ_BUILD_FUZZ_TESTS=ON --install-prefix=/src/minizip-ng/fuzzdrivergpt-install -DCMAKE_VERBOSE_MAKEFILE=ON
make clean
...
cp -r /src/minizip-ng/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/minizip-ng/fuzzdrivergpt-install/lib ${INSTALL}/lib
