...
cd /src/cups
make clean
./configure --enable-static --disable-shared --with-dnssd=no --prefix=/src/cups/fuzzdrivergpt-install
make install
EOF
set +eux
...
cp -r /src/cups/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/cups/fuzzdrivergpt-install/lib ${INSTALL}/lib
cp -r /src/cups/fuzzdrivergpt-install/lib64 ${INSTALL}/lib64
