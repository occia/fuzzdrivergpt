...
cd /src/pupnp/build
make clean
cmake -DFUZZER=ON -DLIB_FUZZING_ENGINE="$LIB_FUZZING_ENGINE" --install-prefix=/src/pupnp/fuzzdrivergpt-install ../
...
cp -r /src/pupnp/fuzzdrivergpt-install/include ${INSTALL}/include

# copy the libs to /root/workspace/fuzzdrivergpt/install/lib
cp -r /src/pupnp/fuzzdrivergpt-install/lib ${INSTALL}/lib
